-- Fázis P4 kiegészítés — a current_question_state()-nek (lásd előző
-- migráció) azt is jeleznie kell, ha az aktuális kérdés megoldása MÁR
-- feltárult (hogy egy visszacsatlakozó csapat a reveal-képernyőt lássa,
-- ne a válasz-UI-t vagy egy örökre "Lezárva" állapotot). Az evaluate_question()
-- (Fázis 5) egy menetben, egyetlen tranzakcióban tölti ki az ÖSSZES adott
-- kérdéshez tartozó answers.is_correct-ot — tehát "van-e legalább egy
-- kiértékelt answers sor erre a kérdésre" megbízható, biztonságos jelzés
-- arra, hogy a feltárás megtörtént, anélkül hogy bármelyik CSAPAT saját
-- válaszát kellene közvetlenül olvasni (amire az anonnak továbbra sincs
-- SELECT policy-ja).
